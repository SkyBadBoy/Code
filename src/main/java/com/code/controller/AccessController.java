...
public class AccessController extends BaseController {
    ...
    public Map queryAccessPage(int status,String search,int pageNumber, int pageSize,HttpServletRequest request) {
        ...
        RabbitUtil.getInstance().OperationLog(request.getHeader("Token"),"查看【Access-queryAccessPage】列表",ReadOnlineService,OperationService,RabbitTemplate,ReadUserService);
        return returnMap;
    }

    @PostMapping("/setAccessStatus")
    @ApiOperation(value = "设置状态")
    public Map setAccessStatus(String data,HttpServletRequest request){
        Map<String,Object> returnMap=new HashMap<>(2);
        ...
        RabbitUtil.getInstance().OperationLog(request.getHeader("Token"),"设置【Access-setAccessStatus】状态",ReadOnlineService,OperationService,RabbitTemplate,ReadUserService);
        return returnMap;
    }

    @GetMapping("/findAccess/{id}")
    @ApiOperation(value = "根据编号查询内容")
    public Map findAccess(@PathVariable("id") String id,HttpServletRequest request){
        Map<String,Object> returnMap=new HashMap<>(2);
        Map<String,Object> queryMap=new HashMap<>(1);
        ...
        RabbitUtil.getInstance().OperationLog(request.getHeader("Token"),"查询【Access-findAccess-"+id+"】内容",ReadOnlineService,OperationService,RabbitTemplate,ReadUserService);
        return returnMap;
    }
    ...
    public Map modifyAccess(String data, HttpServletRequest request) {
        ...
        RabbitUtil.getInstance().OperationLog(request.getHeader("Token"),"修改【Access-modifyAccess-"+obj.getID()+"】内容",ReadOnlineService,OperationService,RabbitTemplate,ReadUserService);
        return returnMap;
    }
