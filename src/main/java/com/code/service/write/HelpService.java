...
import com.code.dao.write.HelpMapper;
 ...
 */
@Service
@CacheConfig(cacheNames="HelpCache") 
@Transactional(propagation=Propagation.REQUIRED,readOnly=false,rollbackFor=Exception.class)
public class HelpService {

   
    @Autowired
	private HelpMapper WriteMapper;

    @Autowired
	private ReadHelpMapper ReadMapper;
 

	@CachePut(key="'Help_'+#p0.ID")  
	@CacheEvict(value = "ReadHelpCache",allEntries = true)
	public Help insert(Help obj){
	...
	@CachePut(key="'Help_'+#p0")  
	@CacheEvict(value = "ReadHelpCache",allEntries = true)
	public Help recoverByID(String id){
		WriteMapper.recoverByID(id);
		return ReadMapper.findById(id);
	}

	@CacheEvict(value = {"ReadHelpCache","HelpCache"},allEntries = true)
